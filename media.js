...
await Promise.all(mediaArr.map(async (media) => {
	...
	console.assert(GPSLatitude.id === 2 && GPSLatitude.value.length === 3 && GPSLatitude.value.every(v => v.length === 2) && GPSLatitude.value[0][1] === 1 && GPSLatitude.value[1][1] === 1 && GPSLatitude.value[0][0] >= 0 && GPSLatitude.value[0][0] < 90, [file, GPSLatitude]); // GPSLatitude.value[2][1] could be 1, 100, 3286.
	...
	console.assert(GPSLongitude.id === 4 && GPSLongitude.value.length === 3 && GPSLongitude.value.every(v => v.length === 2) && GPSLongitude.value[0][1] === 1 && GPSLongitude.value[1][1] === 1 && GPSLongitude.value[0][0] >= 0 && GPSLongitude.value[0][0] < 180, [file, GPSLongitude]); // GPSLongitude.value[2][1] could be 100, 625.
